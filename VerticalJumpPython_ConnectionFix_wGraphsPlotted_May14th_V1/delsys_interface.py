...
class DelsysInterface:
    ...
    @property
    def is_mock_tcp_lab(self) -> bool:
        """True when EMG is fed from the local TCP lab harness (not Trigno AeroPy)."""
        return bool(self._mock_tcp_active)
    ...
    def initialize(self):
        ...
        """
        if os.environ.get("VERTICAL_JUMP_MOCK_DEVICES") == "1":
            return self._initialize_mock_tcp_stream()
    ...
    def _mock_bridge_dir(self) -> Path:
        return Path(__file__).resolve().parent / "MockServerDeviceFolder"

    def _initialize_mock_tcp_stream(self) -> bool:
        """Lab mode: treat JSON-line TCP stream as verified Delsys hardware."""
        mock_dir = self._mock_bridge_dir()
        if str(mock_dir) not in sys.path:
            sys.path.insert(0, str(mock_dir))
        import mock_client_bridge  # noqa: WPS433 (runtime path for lab harness)

        host = os.environ.get("VERTICAL_JUMP_MOCK_DEVICE_HOST", "127.0.0.1")
        port = int(os.environ.get("VERTICAL_JUMP_MOCK_DEVICE_PORT", "8765"))
        mock_client_bridge.ensure_client(host, port)
        self.python_start_time = time.time()
        self.hardware_start_time = self.python_start_time
        self.time_offset = 0.0
        self.is_connected = True
        self.simulation_mode = False
        self._mock_tcp_active = True
        print(
            "DelsysInterface mock TCP mode (VERTICAL_JUMP_MOCK_DEVICES=1): "
            f"stream {host}:{port} presented as hardware EMG path"
        )
        return True

    def _get_mock_tcp_emg_with_timestamps(self) -> dict:
        mock_dir = self._mock_bridge_dir()
        if str(mock_dir) not in sys.path:
            sys.path.insert(0, str(mock_dir))
        ...
        hardware_time = python_time + self.time_offset
        sample = mock_client_bridge.latest_sample()
        if sample is not None:
            left = float(sample.get("emg_l", 0.0))
            right = float(sample.get("emg_r", 0.0))
        else:
            t = python_time
            left = abs(np.sin(t * 2.0)) * 0.12 + 0.02
            right = abs(np.cos(t * 2.0)) * 0.12 + 0.02
        acc1_x = np.sin(python_time * 0.5) * 0.05
        acc1_y = np.cos(python_time * 0.7) * 0.05
        acc1_z = 0.9
        acc2_x = np.sin(python_time * 0.6 + 1.0) * 0.05
        acc2_y = np.cos(python_time * 0.8 + 1.5) * 0.05
        acc2_z = 0.9
        return {
    ...
    def get_emg_data_with_timestamps(self):
        ...
        if self._mock_tcp_active:
            return self._get_mock_tcp_emg_with_timestamps()
