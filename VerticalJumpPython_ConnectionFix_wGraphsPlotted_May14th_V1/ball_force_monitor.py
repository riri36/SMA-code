...
def _ball_first_sample_wait_s() -> float:
    """Wall-clock budget to see the first force sample after ``reader.start()``.

    ``HapticBallReader.start()`` can block until GATT connects, but the first
    notify may still arrive slightly later. Keep this at least on the order of
    ``scan_timeout_s`` so verify does not give up while BLE is still ramping.
    """
    return max(
        float(BALL_CONFIG["connection_verify_timeout_s"]),
        float(BALL_CONFIG["scan_timeout_s"]) + 8.0,
        float(BALL_CONFIG.get("bleak_connect_timeout_s", 60.0)) + 15.0,
    )
...
class BallForceMonitor:
        ...
        *,
        ...
        if self._mock_tcp_devices:
            ...
            self.reader = HapticBallReader(
                ...
                device_name=BALL_CONFIG["device_name"],
                scan_timeout_s=float(BALL_CONFIG["scan_timeout_s"]),
                connect_timeout_s=float(BALL_CONFIG.get("bleak_connect_timeout_s", 60.0)),
                pair_before_connect=bool(BALL_CONFIG.get("bleak_pair_before_connect", False)),
                queue_size=int(BALL_CONFIG["queue_size"]),
                keep_duration=float(BALL_CONFIG["keep_duration"]),
            )
        ...
        self.last_sample_perf: Optional[float] = None
    ...
    def connect(self) -> bool:
        ...
        if self._mock_tcp_devices:
            ...
            deadline = time.time() + _ball_first_sample_wait_s()
            while time.time() < deadline:
                samples = self.reader.get_data()
                print("[ball debug]", samples)
    ...
    def _record_sample(self, sample_ts: float, force: float) -> None:
        self.last_sample_perf = sample_ts
