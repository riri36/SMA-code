...
class AdvancedCalibrationSystem:
    ...
    def _store_trial_data(self, trial_num, phase, left_samples, right_samples, timestamps):
        ...
        trial_data = {
            ...
            'left_peak': float(np.max(left_samples)) if left_samples else 0.0,
            'right_peak': float(np.max(right_samples)) if right_samples else 0.0,
    ...
    def _finalize_calibration(self):
        ...
        # FUSION ADDITION: baseline uses trial means; MVC uses peak amplitude.
        baseline_left = float(np.mean([t['left_mean'] for t in baseline_trials]))
        baseline_right = float(np.mean([t['right_mean'] for t in baseline_trials]))
        mvc_left_peak = float(max(t['left_peak'] for t in mvc_trials))
        mvc_right_peak = float(max(t['right_peak'] for t in mvc_trials))

        default_percent = CALIBRATION_CONFIG['default_mvc_threshold_percent']
        threshold_left = baseline_left + (default_percent / 100.0) * (mvc_left_peak - baseline_left)
        threshold_right = baseline_right + (default_percent / 100.0) * (mvc_right_peak - baseline_right)
        ...
        self.final_calibration.update({
            ...
            'mvc_left': mvc_left_peak,
            'mvc_right': mvc_right_peak,
            'mvc_left_peak': mvc_left_peak,
            'mvc_right_peak': mvc_right_peak,
            'threshold': threshold,
            'mvc_threshold_percent': default_percent,
            'quality_score': 0.9
        })
        self._generate_calibration_warnings()
        
        print(f"🎯 CALIBRATION COMPLETE!")
        print(f"   Baseline: L={baseline_left:.4f}, R={baseline_right:.4f}")
        print(f"   MVC peak: L={mvc_left_peak:.4f}, R={mvc_right_peak:.4f}")
    ...
    def _generate_calibration_warnings(self):
        ...
        # Check MVC amplitude
        max_left_mvc = max(t['left_peak'] for t in self.calibration_data['mvc']['trials'])
        max_right_mvc = max(t['right_peak'] for t in self.calibration_data['mvc']['trials'])
