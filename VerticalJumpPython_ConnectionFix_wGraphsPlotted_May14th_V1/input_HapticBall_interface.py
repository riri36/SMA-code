...
"""
from typing import Any, Coroutine, Deque, List, Optional, Tuple
...
def _asyncio_run_bleak(coro: Coroutine[Any, Any, Any]) -> Any:
    """Run Bleak coroutines in an isolated event loop.

    On Windows, ``asyncio.run`` installs a **Proactor** loop. Bleak's WinRT
    backend is unreliable inside **daemon threads** with that loop (exceptions
    with empty messages, hangs, or ``BLE worker failed``). A **Selector** loop
    in this thread matches Bleak's documented Windows guidance for non-main
    threads (e.g. pygame + lab COM on the main thread).
    """
...
class HapticBallReader:
    ...
    def __init__(
        ...
        pair_before_connect: Optional[bool] = None,
        queue_size: int = 1000,
        ...
        self.connect_timeout_s = float(
            ...
            else BALL_CONFIG.get("bleak_connect_timeout_s", 60.0)
        )
        self.pair_before_connect = (
            bool(pair_before_connect)
            if pair_before_connect is not None
            else bool(BALL_CONFIG.get("bleak_pair_before_connect", False))
        )
        ...
        self._hw_sample_print_last_wall = 0.0
        self._hw_sample_print_last_force: Optional[float] = None

    def probe_hardware(self) -> bool:
        """FUSION ADDITION: scan for hardware without starting sample production.

        On Windows, Bleak's WinRT stack must not run ``asyncio.run`` on the same
        thread that already hosts a GUI/COM STA apartment (e.g. pygame main
        thread after ``pythonnet``/Delsys CLR init). Run the scan on a plain
        worker thread so the event loop is not tied to that apartment.
        """
        outcome: dict[str, object] = {}

        def runner() -> None:
            try:
                outcome["ok"] = bool(_asyncio_run_bleak(self._scan_for_device()))
            except Exception as exc:
                outcome["err"] = exc

        thread = threading.Thread(target=runner, name="hapticball-probe", daemon=True)
        thread.start()
        join_timeout = float(self.scan_timeout_s) + 5.0
        thread.join(timeout=join_timeout)
        if thread.is_alive():
            self.last_error = "HapticBall hardware probe timed out"
            logger.warning("%s", self.last_error)
            return False
        err = outcome.get("err")
        if err is not None:
            self.last_error = str(err)
            logger.warning("HapticBall hardware probe failed: %s", err)
            return False
    ...
    def start(self, *, stream: bool = True) -> None:
        ...
        if self.simulate:
            ...
            # Budget must cover scan + Bleak connect (includes GATT discovery; can be tens of seconds on WinRT).
            budget_s = float(self.scan_timeout_s) + float(self.connect_timeout_s) + 25.0
            deadline = time.time() + budget_s
            while (
                time.time() < deadline
                and not self.hardware_connected
                and self._running.is_set()
                and self._ble_thread.is_alive()
            ):
                time.sleep(0.05)
            if not self.hardware_connected:
                # Keep a BLE worker exception (often empty str(exc)); do not mask it.
                if not (self.last_error or "").strip():
                    self.last_error = f"Device '{self.device_name}' not found"
                logger.warning(
                    "HapticBall not connected; falling back to simulation (%s)",
                    self.last_error,
                )
                self.simulate = True
    ...
    def _append_sample(self, device_ts: float, force: float) -> None:
        ...
        self._maybe_print_hardware_sample(float(force))

    def _maybe_print_hardware_sample(self, force: float) -> None:
        """Optional console trace for real BLE notifications (see BALL_CONFIG)."""
        if self.simulate:
            return
        if not BALL_CONFIG.get("print_ball_hardware_samples", False):
            return
        now = time.time()
        interval_s = max(0.05, float(BALL_CONFIG.get("print_ball_hardware_interval_s", 0.25)))
        min_delta = float(BALL_CONFIG.get("print_ball_hardware_min_delta", 0.01))
        changed = (
            self._hw_sample_print_last_force is None
            or abs(force - self._hw_sample_print_last_force) >= min_delta
        )
        periodic = now - self._hw_sample_print_last_wall >= interval_s
        if not (changed or periodic):
            return
        print(f"[HapticBall HW] force={force:.4f}")
        self._hw_sample_print_last_force = force
        self._hw_sample_print_last_wall = now
    ...
    def _ble_loop(self) -> None:
        try:
           _asyncio_run_bleak(self._connect_and_stream())
        except Exception as exc:
            detail = str(exc).strip() or repr(exc)
            self.last_error = detail
            logger.warning(
                "HapticBall BLE worker failed: %r",
                exc,
                exc_info=True,
            )
            self.hardware_connected = False
    ...
    async def _connect_and_stream(self) -> None:
        ...
        async with BleakClient(
            ...
            # Best-effort cleanup; the link may already be down.
