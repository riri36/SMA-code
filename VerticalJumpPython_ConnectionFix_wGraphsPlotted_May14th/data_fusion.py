...
from input_Delsys_interface import DelsysFusionHub
...
class DataFusion:
  ...
  def __init__(
    ...
    self.delsys_fusion_hub: Optional[DelsysFusionHub] = None
  ...
  def _init_readers(self) -> None:
    ...
    if emg_sensors:
      # FUSION ADDITION: prefer the session DelsysInterface over sim-only hubs.
      if self.delsys_interface is not None:
        self.delsys_fusion_hub = DelsysFusionHub(
          self.delsys_interface,
          self.delsys_sensor_map,
          left_sensor=self.left_sensor,
          right_sensor=self.right_sensor,
          simulate=self.simulate_delsys,
        )
        while not self.delsys_fusion_hub.initialized:
          time.sleep(0.01)
        self.delsys_fusion_hub.start()
        time.sleep(0.2 if self.simulate_delsys else 1.0)
        for sensor_name in emg_sensors:
          self.readers[sensor_name] = self.delsys_fusion_hub.sensor_reader(sensor_name)
  ...
  def _fusion_frame_loop(self) -> None:
    ...
    while not self.stop_event.is_set():
      ...
      if latest_ts is not None:
        # FUSION ADDITION: fusion frames use perf_counter-aligned processing time.
        frame_data["ts"] = t_target
        frame_data["sample_ts"] = latest_ts
  ...
  def stop_fusion(self) -> None:
    self.stop_event.set()
    if self.delsys_fusion_hub is not None:
      try:
        self.delsys_fusion_hub.stop()
        self.delsys_fusion_hub.close()
      except Exception:
        pass
    elif self.delsys_hub is not None:
      try:
