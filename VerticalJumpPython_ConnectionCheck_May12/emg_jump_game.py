...
from python_pygame_version import Player, Pipe, Camera, GROUND_HEIGHT, GRASS_COLOR, GROUND_COLOR
...
class EMGGameController:
    ...
    def ensure_ball_connection(self):
        ...
        connected = self.connect_ball(simulate=False)
        if connected:
            return True
        if CONNECTION_CONFIG["allow_ball_simulation"]:
            return self.connect_ball(simulate=True)
        return False
    ...
    def _load_existing_calibration(self, calibration_file):
        ...
        try:
            ...
            self.calibration_values = {
                ...
                'mvc_left': calibration_data.get('mvc_left', 0.8),
                'mvc_right': calibration_data.get('mvc_right', 0.8),
    ...
    def threshold_from_mvc_percent(self, percent, calibration_values=None):
        ...
        for side in ('left', 'right'):
            ...
            mvc = float(values.get(f'mvc_{side}', 0.8))
            span = max(mvc - baseline, 1e-6)
            channel_thresholds.append(baseline + (float(percent) / 100.0) * span)
        return max(channel_thresholds)

    @staticmethod
    def estimate_mvc_threshold_percent(calibration_values):
        """Estimate MVC-range percent from the current threshold value."""
        threshold = float(calibration_values.get('threshold', 0.3))
        channel_percents = []
        for side in ('left', 'right'):
            baseline = float(calibration_values.get(f'baseline_{side}', 0.05))
            mvc = float(calibration_values.get(f'mvc_{side}', 0.8))
            span = max(mvc - baseline, 1e-6)
    ...
    def _threshold_is_valid(self, new_threshold):
        ...
        mvcs = [
            float(self.calibration_values.get('mvc_left', 0.8)),
            float(self.calibration_values.get('mvc_right', 0.8)),
        ]
    ...
    def apply_mvc_threshold_percent(self, percent):
        """Apply MVC-range percent to the calibrated threshold."""
    ...
    def handle_calibration_message(self, data):
        ...
        if msg_type == 'calibration_phase_started':
            ...
            self.calibration_values = {
                ...
                'mvc_left': result.get('mvc_left', 0.8),
                'mvc_right': result.get('mvc_right', 0.8),
            ...
            self.save_calibration_data(result)
    ...
    def verify_emg_connection(self, accept_simulation=False):
        ...
        self._sync_delsys_connection_flags()
        if self.delsys_connected:
            self.emg_connection_verified = True
            self.emg_using_simulation = False
        elif (
            self.emg_using_simulation
            and accept_simulation
            and CONNECTION_CONFIG["allow_emg_simulation"]
        ):
            self.emg_connection_verified = True
        else:
            self.emg_connection_verified = False
    ...
    def verify_ball_connection(self):
        ...
        if self.ball_monitor and self.ball_monitor.connection_status in ("connected", "simulated"):
            self.ball_connection_verified = True
    ...
    def connect_ball(self, simulate=False):
    ...
    def save_session_data(self):
        ...
        if self.raw_data_buffer:
            ...
            with open(raw_file, 'w', newline='') as f:
                writer = csv.DictWriter(f, fieldnames=['timestamp', 'left_raw', 'right_raw'])
                writer.writeheader()
        ...
        if self.processed_emg_buffer:
            ...
            with open(proc_file, 'w', newline='') as f:
                writer = csv.DictWriter(f, fieldnames=['timestamp', 'unityTimestamp', 'localTimestamp',
                                                       'emg1', 'emg2', 'rms1', 'rms2', 
...
class IntegratedEMGGame:
    ...
    def handle_connection_verify_events(self, event):
        ...
        if event.type != pygame.KEYDOWN or self.ball_connect_in_progress:
            return
        if event.key == pygame.K_e:
            ...
            self.emg_controller.ball_connection_verified = False
            self.emg_controller.ball_using_simulation = False
            self.emg_controller.ball_hardware_connected = False
            self.emg_controller._refresh_ball_connection_message()
    ...
    def _prepare_threshold_adjust_screen(self):
        ...
        values = self.emg_controller.calibration_values
        self.threshold_percent_value = float(
            values.get(
                'mvc_threshold_percent',
        ...
        self.threshold_percent_input = ""
        self.threshold_percent_error = ""
        self.emg_controller.apply_mvc_threshold_percent(self.threshold_percent_value)
    ...
    def handle_threshold_adjust_events(self, event):
        ...
        if event.key == pygame.K_RETURN:
            ...
            self.threshold_percent_input = ""
            self.threshold_percent_error = ""
            if self.emg_controller.apply_mvc_threshold_percent(default_percent):
    ...
    def _apply_threshold_percent_input(self):
        ...
        if not min_percent <= percent <= max_percent:
            ...
            return False
        if self.emg_controller.apply_mvc_threshold_percent(percent):
    ...
    def draw_calibration(self):
        ...
        inst_rect = inst_surface.get_rect(center=(self.SCREEN_WIDTH//2, 250))
        ...
        if self.calibration_progress > 0:
            fill_width = int(bar_width * self.calibration_progress)
        ...
        if force_value is not None:
            ...
            force_rect = force_text.get_rect(center=(self.SCREEN_WIDTH//2, 395))
        ...
        for index, ball in enumerate(self.calibration_balls):
            ...
            color = ball["active_color"] if active else ball["color"]
            center = (int(ball["x"]), int(ball["y"]))
            pygame.draw.circle(self.screen, color, center, ball["radius"])
            label_text = self.small_font.render(ball["label"], True, (230, 230, 230))
            label_rect = label_text.get_rect(center=(center[0], center[1] + ball["radius"] + 14))
    ...
    def draw_threshold_adjust(self):
        ...
        input_box = pygame.Rect(120, 130, 160, 40)
        pygame.draw.rect(self.screen, (255, 255, 255), input_box, 2)
        ...
        current_threshold = self.emg_controller.calibration_values['threshold']
        threshold_text = self.small_font.render(
            f"Threshold: {current_threshold:.3f}",
            True,
            (255, 255, 0),
        )
        threshold_rect = threshold_text.get_rect(center=(self.SCREEN_WIDTH//2, 205))
        self.screen.blit(threshold_text, threshold_rect)
        ...
        if self.threshold_percent_error:
            ...
            error_rect = error_text.get_rect(center=(self.SCREEN_WIDTH//2, 380))
        ...
        guide_rect = guide_text.get_rect(center=(self.SCREEN_WIDTH//2, 450))
        ...
        guide_rect2 = guide_text2.get_rect(center=(self.SCREEN_WIDTH//2, 478))
    ...
    def draw_connection_verify(self):
        ...
        if self.emg_controller.emg_connection_verified:
            emg_color = (0, 255, 0) if emg_status == "Hardware" else (255, 255, 0)
        else:
            emg_color = (255, 100, 100)
        emg_text = self.small_font.render(f"EMG: {emg_status}", True, emg_color)
        emg_rect = emg_text.get_rect(center=(self.SCREEN_WIDTH//2, 170))
        self.screen.blit(emg_text, emg_rect)

        ball_status = self.emg_controller._ball_status_label()
        if self.ball_connect_in_progress:
            ball_status = "Connecting..."
            ball_color = (255, 255, 0)
        elif self.emg_controller.ball_connection_verified:
            ball_color = (0, 255, 0) if ball_status == "Hardware" else (255, 255, 0)
        else:
            ball_color = (255, 100, 100)
        ball_text = self.small_font.render(f"Ball: {ball_status}", True, ball_color)
        ball_rect = ball_text.get_rect(center=(self.SCREEN_WIDTH//2, 210))
        self.screen.blit(ball_text, ball_rect)
        ...
        if force_value is not None:
            force_text = self.small_font.render(
                f"Live force: {force_value:.3f}",
                True,
                (255, 255, 255),
            )
            force_rect = force_text.get_rect(center=(self.SCREEN_WIDTH//2, 245))
            self.screen.blit(force_text, force_rect)

        instructions = [
            "E refresh EMG path",
            "C connect ball (auto-sim if absent)",
            "R retry ball",
            "ENTER when both paths verified",
        ]
        y = 310
        for line in instructions:
            text = self.small_font.render(line, True, (200, 200, 200))
    ...
    def draw_menu(self):
        ...
        if self.emg_controller.calibration_complete:
            ...
            self.screen.blit(thresh_text, thresh_rect)
    ...
    def draw_gameplay(self):
        """Draw gameplay (from original game)"""
        # Draw ground
        if self.camera.y < 100:
            ground_y = self.SCREEN_HEIGHT - GROUND_HEIGHT + self.camera.y
            pygame.draw.rect(self.screen, GRASS_COLOR, (0, ground_y, self.SCREEN_WIDTH, 20))
            pygame.draw.rect(self.screen, GROUND_COLOR, (0, ground_y + 20, self.SCREEN_WIDTH, GROUND_HEIGHT - 20))
        ...
        mode_label = mode_labels.get(self.emg_controller.control_mode, self.emg_controller.control_mode)
        mode_text = self.small_font.render(f"Control: {mode_label}", True, (255, 255, 255))
        mode_rect = mode_text.get_rect(topright=(self.SCREEN_WIDTH - 10, 10))
        self.screen.blit(mode_text, mode_rect)

        emg_label = "EMG Active" if self.emg_controller.control_mode == "emg" else "EMG Logging"
        emg_text = self.small_font.render(emg_label, True, (0, 255, 0))
        emg_rect = emg_text.get_rect(topright=(self.SCREEN_WIDTH - 10, 34))
        self.screen.blit(emg_text, emg_rect)
        ...
        if force_value is not None:
            ...
            force_rect = force_text.get_rect(topright=(self.SCREEN_WIDTH - 10, 58))
            self.screen.blit(force_text, force_rect)
    ...
    def draw_session_end(self):
        ...
        self.screen.blit(saving_text, saving_rect)
