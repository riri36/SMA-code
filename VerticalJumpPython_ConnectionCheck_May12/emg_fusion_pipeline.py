...
class EMGFusionPipeline:
  ...
  def _normalize(self, side: str, rms_value: float) -> float:
    ...
    mvc = float(self.calibration_values.get(mvc_key, 0.8))
  ...
  def _emit_jump(self, trigger: FusionTriggerState, value: float, features: Dict[str, float]) -> None:
    ...
    event = {
      "timestamp": time.time(),
      "session_elapsed_s": time.perf_counter() - self.session_start_perf,
      "perf_counter": time.perf_counter(),
      "left_value": left_value,
      "right_value": right_value,
      "force_value": float(features.get("ball.force.rms", value)),
      "threshold": trigger.threshold,
      "trigger_id": trigger.trigger_id,
      "source": trigger.source,
      "simulated": self.simulate_ball if trigger.source == "ball.force.rms" else self.simulate_delsys,
    }
    ...
    if self.on_jump:
      self.on_jump(event)
    if trigger.source == "ball.force.rms":
